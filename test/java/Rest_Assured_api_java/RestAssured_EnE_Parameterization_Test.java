...
public class RestAssured_EnE_Parameterization_Test{
    @Test(dataProvider = "BookData")
    public void addBook(String bookData,String bookID){
            // pass the base URI
            RestAssured.baseURI = "https://rahulshettyacademy.com";
            //Enter all inputs
            String response = given().log().all().header("content-Type", "application/json")
                    //.body(new String(Files.readAllBytes(Paths.get("<json file path>"))))
                    .body(Payload_Data.addBook_payload(bookData, bookID))
                    //Enter the endpoints
                    .when()
                    //Post Method
    ...
    public Object[][] getData(){
        return new Object[][] {{"Ind","292426"},{"Aus","242926"},{"RSA","242729"},{"NZ","453211"}};
